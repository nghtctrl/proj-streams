...
To prepare our dataset for visualization and modeling purposes, we will transform the columns and values into more human-readable format. Some columns have values with multiple answers (i.e., survey respondents were able to select multiple answers to a question). For these columns, further data wrangling process may be necessary depending on our partner's needs.
